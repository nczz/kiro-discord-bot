...
func commandDefaultMemberPermissions(name string) *int64 {
	switch name {
	case "audit", "mcp", "steering", "cwd", "start", "agent", "webhook", "webshare", "cron", "cron-list", "cron-run", "cron-prompt", "monitor-list", "monitor-run", "monitor-prompt", "memory", "flashmemory", "clear":
...
func commandResponseVisibility(name string, args string) commandVisibility {
	...
	case "help", "status", "usage", "usage-history", "doctor", "audit", "mcp", "steering", "cwd", "models", "agent",
		"webhook", "webshare", "session", "memory", "flashmemory", "a2a", "cron-list", "cron-run", "cron-prompt", "monitor-list", "monitor-run", "monitor-prompt", "remind":

...
func buildPromptThreadWithMentions(text string, attachments []string, channelID, threadID, guildID, username, userID, peerContext string, mentionRefs []discordmention.Ref) string {
	...
	sb.WriteString("Do not inspect or mutate raw bot state files or databases to answer ordinary user requests. Use available exposed bot MCP tools such as bot_a2a_peers, bot_a2a_trust_peer, bot_a2a_delegate, and bot_a2a_task_status. Retired expert A2A policy tools are not available in bot-tools; if a requested policy operation cannot be done with the high-level tools, report that a channel manager must use a trusted admin path instead of editing database files directly.\n")
	...
	sb.WriteString("For time/date/weekday questions, use the [Current datetime] block below for current facts. For calculated date ranges, relative periods, month/week boundaries, specific weekdays, nth week of a month, or schedule-sensitive answers, translate the user's date phrase into structured bot_resolve_date_range fields and call that tool when available. Examples: 明天 => range_type=day offset=1; 下個月第二週 => range_type=month_week offset=1 week_index=2; 過去7天 => range_type=relative_days days=7 direction=past. The structured fields are language-neutral; do not pass natural-language date text to the MCP tool. Do not calculate weekdays, month boundaries, or relative ranges from model memory when bot time tools are available. Use the [Current datetime] timezone for date/time reasoning. Do not append a timezone to every date/time answer. Mention it only when the user asks, when confirming reminder/schedule/cron times, or when a different timezone could change the date, range, or clock time. This affects reply wording only; still call bot_resolve_date_range for calculated ranges. Do not expose CRON_TIMEZONE, timezone_source, or [Current datetime] in normal replies.\n")

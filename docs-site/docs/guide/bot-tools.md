...
Use `bot_a2a_peers` before claiming another bot can receive work. The response is scoped to the current Discord guild/channel runtime: `peerPolicy.enabled=true` means the policy summary is active, `peerPolicy.inboundAllowedRuntimes` shows exact runtimes allowed to send work into this channel, `peerPolicy.legacyInboundAllowedAgents` shows legacy bot-agent grants separately, `peerPolicy.outboundDelegateTargets` shows explicit outbound targets, per-peer `inboundAllowed=true` means that peer may send work here, and `delegationAllowed=true` means the current channel policy explicitly allows `bot_a2a_delegate` for that runtime. The peer `skills` array is derived from outbound policy; if the peer card does not advertise a callable skill, the normal task skill is reported as `task` instead of blocking delegation. The remote runtime can still reject by its own receiver policy, quota, or delivery checks. `trusted=true` only describes trust display state; same-channel collaboration also requires receiver policy and Discord permission readiness.

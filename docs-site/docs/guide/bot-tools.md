...
| `bot_a2a_peers` | Read | List known A2A runtime peers, callable channel refs, policy-derived callable skills, wakeability, and delivery readiness. |
...
Use `bot_a2a_peers` before claiming another bot can receive work. The response is scoped to the current Discord guild/channel runtime: `peerPolicy.enabled=true` means the policy summary is active, `peerPolicy.inboundAllowedRuntimes` shows exact runtimes allowed to send work into this channel, `peerPolicy.legacyInboundAllowedAgents` shows legacy bot-agent grants separately, `peerPolicy.outboundDelegateTargets` shows explicit persistent outbound targets, per-peer `inboundAllowed=true` means that peer may send work here, and `delegationAllowed=true` means the current channel policy explicitly allows `bot_a2a_delegate` for that runtime. The peer `skills` array is derived from outbound policy; if the peer card does not advertise a callable skill, the normal task skill is reported instead and does not block policy-authorized delegation. Remote runtime may still reject on its own receiver policy, quota, or delivery checks. `trusted=true` is only trust display state; same-channel collaboration still needs receiver policy and Discord permission readiness.
...
For sender-side delegation, use `bot_a2a_delegate` with `target_agent` and `message` for normal tasks. Persistent/scheduled delegation requires the current channel's outbound `delegate_targets` policy. A direct human message, thread message, or `/a2a ask` may also queue a one-shot task to a discovered, non-stale runtime without mutating `delegate_targets`; cron, remote A2A, and bot-handoff jobs still fail closed without persistent policy. Omitted or unadvertised skills default to the normal `task` capability. Same Discord channel/thread means conversation collaboration, while a different channel means the receiver works in its own channel/thread and returns the result. A successful call means the task was durably queued, not accepted or completed. Use `bot_a2a_task_status` with `local_id`, `task_id`, or `message_id` for authoritative state before reporting success.

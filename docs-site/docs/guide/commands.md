...
| `/silent [on|off|compact|full|folded]` | Control progress output visibility. Default is `folded`: one live progress card while keeping the final answer unchanged. `on`/`compact` posts compact tool starts; `off`/`full` posts detailed tool output. |

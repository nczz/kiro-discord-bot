...
`kiro-discord-bot` connects Discord to Kiro CLI agents through ACP over stdio. It is designed for teams that want an agent to understand a project, keep useful context, and work from Discord without losing operational control.
...
2. Create `.env` with the Discord token, guild ID, default CWD, data directory, and Kiro settings.
...
When a channel is not initialized, the bot holds normal messages back and asks a channel manager to open the private `/cwd` setup panel. Setup chooses or creates a project under `DEFAULT_CWD`, creates `.kiro/steering/` when needed, and enables the built-in `bot-tools` MCP server with a safe default allowlist.
...
- Put project rules, architecture, and workflow guidance in `.kiro/steering/*.md`.

...
兩個 engines 都是外部專案。這個 repository 不維護 `kiro-cli` 或 `omp`；它提供的是包在這些 CLI 外面的 Discord control plane。安裝、認證與更新請使用各自 upstream 工具。基本命令與連結見 [安裝](installation.md)。
...
OMP 官方專案網站：[omp.sh](https://omp.sh/)。

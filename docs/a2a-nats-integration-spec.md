...
- current channel policy explicitly allows the target runtime/channel through `delegate_targets`, or the request is a direct human message/thread/slash one-shot targeting a discovered non-stale runtime; remote receiver policy remains authoritative。
...
→ bot_a2a_peers as needed
→ bot_a2a_trust_peer with target_agent only
...
3. `bot_a2a_delegate` requires explicit outbound `delegate_targets` policy for persistent/scheduled delegation. Direct human message/thread/slash requests may queue a one-shot task to a discovered non-stale runtime without mutating sender policy; cron, remote A2A, and bot-handoff jobs still fail closed without persistent policy. Omitted or unadvertised skills default to `task`. Remote receiver policy remains authoritative and the call may require confirmation for remote data egress, attachments, sensitive skills, or transparent/co-present delivery。
...
7. Retired expert policy plan/apply/readiness bot-tools are not default-enabled and are not registered in the normal bot-tools MCP surface。
8. The agent may explain proposed changes, but cannot bypass confirmation or permission checks by wording。

### 18.3 Slash command fallback

Slash commands remain available for bootstrap, troubleshooting, and users who prefer explicit commands。They call the same internal service methods as bot-tools and intentionally expose only the high-level A2A workflow.

| Command | Permission | Visibility | Effect |
|---|---|---|---|
| `/a2a peers` | normal user | ephemeral | list known peer skills |
| `/a2a allow` | ManageChannels | ephemeral/public status update | allow one exact peer runtime to send normal work here |
| `/a2a ask` | normal user, policy-gated | public ack + thread updates | send a normal text task to a peer runtime |
...
| `/a2a revoke` | ManageChannels | ephemeral/public status update | stop allowing one exact peer runtime to send work here |
...
- `bot_a2a_trust_peer` is receiver-side consent only: it immediately adds the exact peer runtime to `accept_from_runtimes`, does not widen `accept_skills`, and rejects relationship, skill, channel reference, transcript, and result visibility fields.
...
### Phase 5: High-level policy UX hardening

- retire expert bot-tools policy plan/apply/readiness surfaces from the normal MCP surface。
- keep receiver consent on `bot_a2a_trust_peer` simple, exact-runtime scoped, and immediate。
- policy audit。
- docs update。

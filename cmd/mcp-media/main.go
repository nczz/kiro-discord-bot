...
func main() {
	...
	syncWait := envDuration("MEDIA_SYNC_WAIT_SEC", 20*time.Second)

	s := newMediaServer(reg, jobs, jobTimeout, syncTimeout, syncWait)
...
func newMediaServer(reg *Registry, jobs *MediaJobStore, jobTimeout, syncTimeout, syncWait time.Duration) *server.MCPServer {
	s := server.NewMCPServer("mcp-media", "1.0.0", server.WithToolCapabilities(false))

	// --- generate_image ---
	s.AddTool(
		mcp.NewTool("generate_image",
			mcp.WithDescription("Generate an image from a text prompt through a managed media job. If it finishes quickly, returns the local file path. If it is still running before the MCP client timeout, returns a job_id; then use get_media_job to retrieve the saved path. Do not retry the same request after receiving a job_id."),
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			job, err := jobs.StartAndWait("image", prompt, m, syncTimeout, syncWait, func(jobCtx context.Context) (*MediaResult, error) {
			...
			if job.Status == mediaJobFailed || job.Status == mediaJobCanceled {
	...
	s.AddTool(
		mcp.NewTool("start_image_generation",
			mcp.WithDescription("Start an asynchronous image generation job and immediately return a job_id. Use this when you explicitly want job-style control from the beginning. Poll with get_media_job until status is succeeded or failed."),
			mcp.WithString("prompt", mcp.Required(), mcp.Description("Image description")),
			mcp.WithString("model", mcp.Description("Model ID (use list_models to see options). Default: "+reg.defaultImage)),
			mcp.WithString("size", mcp.Description("Image size. Gemini: 512/1K/2K/4K. OpenAI: 1024x1024/1024x1792/1792x1024")),
			mcp.WithString("aspect_ratio", mcp.Description("Aspect ratio for Gemini models: 1:1, 16:9, 9:16, 3:2, 2:3, 4:3, 3:4, etc.")),
	...
	s.AddTool(
		mcp.NewTool("get_media_job",
			...
			mcp.WithString("job_id", mcp.Required(), mcp.Description("Job ID returned by a managed generate/edit/TTS tool or a start_* media job tool")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			job, err := jobs.StartAndWait("image_edit", prompt, m, syncTimeout, syncWait, func(jobCtx context.Context) (*MediaResult, error) {
				return p.EditImage(jobCtx, imgPath, prompt, m)
			})
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
			text := formatAdaptiveMediaJob(job, "Edited image saved")
			if job.Status == mediaJobFailed || job.Status == mediaJobCanceled {
				return mcp.NewToolResultError(text), nil
			}
			return mcp.NewToolResultText(text), nil
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			job, err := jobs.StartAndWait("video", prompt, m, syncTimeout, syncWait, func(jobCtx context.Context) (*MediaResult, error) {
				return p.GenerateVideo(jobCtx, prompt, m, imgPath)
			})
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
			text := formatAdaptiveMediaJob(job, "Video saved")
			if job.Status == mediaJobFailed || job.Status == mediaJobCanceled {
				return mcp.NewToolResultError(text), nil
			}
			return mcp.NewToolResultText(text), nil
	...
	s.AddTool(
		mcp.NewTool("generate_music",
			mcp.WithDescription("Generate a music track through a managed media job. If it finishes quickly, returns the local file path. If it is still running before the MCP client timeout, returns a job_id; then use get_media_job to retrieve the saved path. Do not retry the same request after receiving a job_id."),
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			job, err := jobs.StartAndWait("music", prompt, m, syncTimeout, syncWait, func(jobCtx context.Context) (*MediaResult, error) {
				return p.GenerateMusic(jobCtx, prompt, m, dur)
			})
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
			text := formatAdaptiveMediaJob(job, "Music saved")
			if job.Status == mediaJobFailed || job.Status == mediaJobCanceled {
				return mcp.NewToolResultError(text), nil
			}
			return mcp.NewToolResultText(text), nil
	...
	s.AddTool(
		mcp.NewTool("text_to_speech",
			mcp.WithDescription("Convert text to speech through a managed media job. If it finishes quickly, returns the local file path. If it is still running before the MCP client timeout, returns a job_id; then use get_media_job to retrieve the saved path. Do not retry the same request after receiving a job_id."),
			mcp.WithString("text", mcp.Required(), mcp.Description("Text to speak")),
			mcp.WithString("model", mcp.Description("Model ID. Default: "+reg.defaultTTS)),
			mcp.WithString("voice", mcp.Description("Voice name. OpenAI: alloy/echo/fable/onyx/nova/shimmer. Gemini: Puck/Charon/Kore/Fenrir/Aoede")),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			job, err := jobs.StartAndWait("tts", text, m, syncTimeout, syncWait, func(jobCtx context.Context) (*MediaResult, error) {
				return p.TextToSpeech(jobCtx, text, m, voice)
			})
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
			resultText := formatAdaptiveMediaJob(job, "Audio saved")
			if job.Status == mediaJobFailed || job.Status == mediaJobCanceled {
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			for _, t := range []string{"image", "video", "music", "tts"} {
				...
				for _, m := range ms {
					def := ""
					if (t == "image" && m.ID == reg.defaultImage) ||
						(t == "video" && m.ID == reg.defaultVideo) ||
						(t == "music" && m.ID == reg.defaultMusic) ||
						(t == "tts" && m.ID == reg.defaultTTS) {
						def = " ★default"
					}
					cost := ""
					if m.CostTier != "" {

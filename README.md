...
The bot also registers a built-in `bot-tools` MCP catalog entry backed by the same bot binary (`mcp-bot`). It exposes bot data-directory metadata tools, scoped audit timeline lookup (`bot_query_audit`), optional safe Discord message egress (`bot_send_message`), safe file egress (`bot_send_file`), and cron management tools (`bot_create_cron`, `bot_list_cron`, `bot_delete_cron`). New channel setup enables `bot-tools` by default with the safe allowlist (`bot_data_summary`, `bot_list_channel_data`, `bot_list_cron`, `bot_query_audit`, `bot_send_file`, `bot_create_cron`) and leaves message egress plus the destructive `bot_delete_cron` tool disabled until a manager explicitly allows them. Ordinary agent replies and final answers should be returned as normal assistant text so the bot can redact, split, and deliver them consistently. Existing channels and external catalog servers still require explicit policy changes through `/mcp manage` or `/mcp enable`.

...
The bot also registers a built-in `bot-tools` MCP catalog entry backed by the same bot binary (`mcp-bot`). It exposes bot data-directory metadata tools, optional safe Discord message egress (`bot_send_message`), safe file egress (`bot_send_file`), and cron management tools (`bot_create_cron`, `bot_list_cron`, `bot_delete_cron`). New channel setup enables `bot-tools` by default with the safe allowlist (`bot_data_summary`, `bot_list_channel_data`, `bot_list_cron`, `bot_send_file`, `bot_create_cron`) and leaves message egress plus the destructive `bot_delete_cron` tool disabled until a manager explicitly allows them. Ordinary agent replies and final answers should be returned as normal assistant text so the bot can redact, split, and deliver them consistently. Existing channels and external catalog servers still require explicit policy changes through `/mcp manage` or `/mcp enable`.
...
- **Built-in bot-tools MCP:** `bot-tools` is served by the bot binary through the `mcp-bot` subcommand. It exposes metadata tools, optional safe Discord message egress, default-enabled safe file egress, and cron management tools. Metadata/list tools are read-only. New channel setup enables the safe default allowlist automatically, including `bot_send_file` but not `bot_send_message`; ordinary agent replies should return as normal assistant text and let the bot handle redaction, splitting, and delivery. Managers can explicitly allow `bot_send_message` through `/mcp manage` for separate Discord notifications or handoffs. `bot_delete_cron` stays disabled until explicitly allowed. `bot_list_channel_data` returns known public channel/thread names when the bot has observed them, without returning message content. `bot_send_message` and `bot_send_file` queue pending egress actions that the main bot delivers to the current channel or thread target only after secret redaction; text files upload as sanitized copies, PDF/DOCX/XLSX documents with extractable readable text upload as extracted and redacted `.txt` copies, and unsupported binary, unreadable, oversized, or over-expanded files are refused instead of uploading the original.

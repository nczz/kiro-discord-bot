...
The bot also registers a built-in `bot-tools` MCP catalog entry backed by the same bot binary (`mcp-bot`). It exposes bot data-directory metadata tools, scoped audit timeline lookup (`bot_query_audit`), optional safe Discord message egress (`bot_send_message`), safe file egress (`bot_send_file`), and cron management tools (`bot_create_cron`, `bot_list_cron`, `bot_delete_cron`). New channel setup enables `bot-tools` by default with the safe allowlist (`bot_data_summary`, `bot_list_channel_data`, `bot_list_cron`, `bot_send_file`, `bot_create_cron`) and leaves audit lookup, message egress, and the destructive `bot_delete_cron` tool disabled until a manager explicitly allows them. Manager-authorized `/audit <prompt>` investigations use a private short-lived agent with only the scoped `bot_query_audit` tool, so channel managers can inspect audit history even when the channel's ordinary MCP policy does not expose audit lookup to agents. Ordinary agent replies and final answers should be returned as normal assistant text so the bot can redact, split, and deliver them consistently. Existing channels and external catalog servers still require explicit policy changes through `/mcp manage` or `/mcp enable`.
...
| `bot_list_cron` | read-only | List scheduled cron jobs for the owning parent channel; thread IDs are normalized to the parent channel |
| `bot_query_audit` | read-only | Query scoped audit timeline rows for the current channel/thread target. It is not a raw SQL tool and cannot read raw tables, `raw_json`, or message content |
| `bot_send_message` | write, non-destructive | Optional tool, not default-enabled. Queue a separate Discord message for bot-side secret redaction, splitting, and delivery to the current channel or thread target |
| `bot_send_file` | write, non-destructive | Default-enabled safe file egress. Queue a local text file for bot-side sanitization, or a PDF/DOCX/XLSX document with extractable readable text for text extraction, redaction, and upload as a sanitized `.txt` copy to the current channel or thread target; long optional content is split before delivery |
| `bot_create_cron` | write, non-destructive | Queue creation of a recurring cron job for the owning parent channel; thread IDs are normalized to the parent channel |
| `bot_delete_cron` | write, destructive | Queue deletion of a cron job; thread IDs are normalized to the parent channel and deletion is accepted only for the owning channel |
...
Use `/mcp status` for a combined catalog and policy checklist. `/mcp enable` exposes the entire selected server. Use `/mcp manage` for tool-level control and MCP reload: the panel scans a server's current `tools/list` through the same MCP policy proxy used by agent runtime, caches those tools in SQLite, exposes select menus for allow/remove operations, and stops active agents so the next run loads the current MCP policy. The raw `allow-tool` and `deny-tool` commands are intentionally not exposed because they require users to type exact tool names and cannot provide a good Discord UX. For macOS LaunchAgent deployments where a private-LAN URL works from an interactive shell but `/mcp manage` scan fails with `no route to host`, see [macOS MCP Networking](docs/macos-mcp-networking.md).
...
- **Built-in bot-tools MCP:** `bot-tools` is served by the bot binary through the `mcp-bot` subcommand. It exposes metadata tools, scoped audit timeline lookup, optional safe Discord message egress, default-enabled safe file egress, and cron management tools. Metadata/list/audit tools are read-only. New channel setup enables the safe default allowlist automatically, including `bot_send_file` but not `bot_query_audit` or `bot_send_message`; manager-authorized `/audit <prompt>` investigations use a private short-lived agent with only the scoped audit lookup tool. Ordinary agent replies should return as normal assistant text and let the bot handle redaction, splitting, and delivery. Managers can explicitly allow `bot_query_audit` or `bot_send_message` through `/mcp manage` when ordinary channel agents should get those tools. `bot_delete_cron` stays disabled until explicitly allowed. `bot_list_channel_data` returns known public channel/thread names when the bot has observed them, without returning message content. `bot_query_audit` is scoped by the bot-tools channel/thread env and returns timeline rows only, not arbitrary SQL results, raw JSON, or message content. `bot_send_message` and `bot_send_file` queue pending egress actions that the main bot delivers to the current channel or thread target only after secret redaction; text files upload as sanitized copies, PDF/DOCX/XLSX documents with extractable readable text upload as extracted and redacted `.txt` copies, and unsupported binary, unreadable, oversized, or over-expanded files are refused instead of uploading the original.
- **Structured Discord mentions:** Agents do not get raw Discord mention tokens as an API. For each user message, the prompt lists verified mention reference placeholders for the requester, structured Discord user mentions present in the source message, and configured peer handoff targets. Final delivery renders only those exact placeholders into Discord mentions and scopes `AllowedMentions` to the rendered users/roles. Raw mention strings guessed or assembled by the agent are escaped and do not ping.
...
- **Cron jobs:** Definitions in `DATA_DIR/cron/cron.json`, execution history in `DATA_DIR/cron/<jobID>/history.jsonl` (includes full agent output). Set `CRON_TIMEZONE` explicitly for deployed services so schedules such as `30 12 * * *` run in the intended local timezone. Cron setup no longer asks for a working directory; scheduled agent work always runs with the owning channel's current CWD, so changing `/cwd` changes subsequent cron executions for that channel. Agent-backed cron runs always create or reuse their dedicated Discord thread; progress and final responses are delivered to that cron thread, while the parent channel only receives the run link. If a manager explicitly enables built-in `bot_send_message` / `bot_send_file` safe egress, those pending deliveries also target the cron thread. Cron management tools always scope jobs to the owning parent channel; if an agent passes the current thread target as `channel_id`, bot-tools normalizes it back to the bound parent channel. Uninitialized channels cannot create, resume, manually run, or execute agent-backed cron jobs. `bot-tools` writes cron create/delete requests as pending JSON actions under `DATA_DIR/cron/pending/`; `CronTask` validates and ingests those actions on scheduler ticks, removes invalid actions, and only deletes jobs when the requested channel owns the job.

...
func TestDefaultSafeToolNamesIncludesOnlyReadSkillTools(t *testing.T) {
	...
	for _, forbidden := range []string{ToolSkillUsageRecord, ToolSkillCreateDraft, ToolSkillInstallDraft, ToolSkillDiscardDraft, ToolSkillsChannelEnable, ToolSkillsChannelDisable, ToolSkillsChannelRemove, ToolSkillsChannelRestore, ToolSkillsChannelRollback, ToolSkillsServerDisable, ToolSkillsServerRemove, ToolSkillsServerRestore, ToolSkillsServerRollback} {
		if seen[forbidden] {
...
func TestSkillCreateDraftRejectsRawHTML(t *testing.T) {
	_, err := skillCreateDraft(context.Background(), t.TempDir(), skillReq(map[string]any{"name": "HTML Skill", "scope_type": skills.ScopeGuild, "guild_id": "guild-1", "content_markdown": "<!doctype html><html><body>not a skill</body></html>", "source_type": "url", "source_ref": "https://gist.github.com/example", "requested_by": "alice"}))
	if err == nil || !strings.Contains(err.Error(), "curated markdown") {
		t.Fatalf("raw HTML was not rejected: %v", err)
	}
}

func TestSkillCreateDraftAllowsMarkdownHTMLExamples(t *testing.T) {
	content := "# Procedure\nAdd this snippet when needed:\n\n```html\n<script src=\"/assets/app.js\"></script>\n<body>example</body>\n```"
	if _, err := skillCreateDraft(context.Background(), t.TempDir(), skillReq(map[string]any{"name": "HTML Example Skill", "scope_type": skills.ScopeGuild, "guild_id": "guild-1", "content_markdown": content, "source_type": "conversation", "requested_by": "alice"})); err != nil {
